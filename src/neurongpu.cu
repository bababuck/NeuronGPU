...
void NewInternSimulationStepKernel(int n_nodes, int conns, bool rev_conns, double t, float time_step){
  ...
  __syncthreads();
  for (unsigned int i=0; i<(n_nodes-1); i++) {
    InternUpdate(0, 0, d_node_vect[i], internal_loop);
  }
  __syncthreads(); 
  InternClearGetSpikeArrays(n_nodes);
...
/*  SpikeBufferUp<<<30, 1024>>>();
  gpuErrchk( cudaPeekAtLastError() );
  gpuErrchk( cudaDeviceSynchronize() ); 
  node_vect_update<<<30,1024>>>(node_vect_.size());
  gpuErrchk( cudaPeekAtLastError() );
  gpuErrchk( cudaDeviceSynchronize() );

  ClearGetSpikeArraysKernel<<<30,1024>>>(node_vect_.size());
  gpuErrchk( cudaPeekAtLastError() );
  gpuErrchk( cudaDeviceSynchronize() );
  InternRunKernel<<<30, 1024>>>();
  gpuErrchk( cudaPeekAtLastError() );
  gpuErrchk( cudaDeviceSynchronize() );

  InternGetSpikesKernel<<<30, 1024>>>(node_vect_.size());
  gpuErrchk( cudaPeekAtLastError() );
  gpuErrchk( cudaDeviceSynchronize() );

  InternSpikeResetKernel<<<30,1>>>();                                                                                                                                                                                                          gpuErrchk( cudaPeekAtLastError() );
  gpuErrchk( cudaDeviceSynchronize() );
  if (net_connection_->NRevConnections()>0) {                                                                                                                                                                                                 
    InternRevSpikeResetKernel<<<30, 1>>>();                                                                                                                                                                                                      gpuErrchk( cudaPeekAtLastError() );
    gpuErrchk( cudaDeviceSynchronize() );
    RevSpikeBufferUp<<<30, 1024>>>( net_connection_->connection_.size(), 0);                                                                                                                                                                  
    gpuErrchk( cudaPeekAtLastError() );
    gpuErrchk( cudaDeviceSynchronize() );
    InternRevRunKernel<<<30, 1024>>>();
  }
  gpuErrchk( cudaPeekAtLastError() );
  gpuErrchk( cudaDeviceSynchronize() );
*/  NewInternSimulationStepKernel<<<30, 1024>>>(node_vect_.size(), net_connection_->connection_.size(), net_connection_->NRevConnections()>0, neural_time_, time_resolution_); 
  gpuErrchk( cudaPeekAtLastError() );
  gpuErrchk( cudaDeviceSynchronize() );
...
/*
  ...
  for (unsigned int i=0; i<node_vect_.size(); i++) {
    if (node_vect_[i]->has_dir_conn_) {
      node_vect_[i]->SendDirectSpikes(neural_time_+internal_loop*time_resolution_, time_resolution_/1000.0);
    }
  }
  }
*/
