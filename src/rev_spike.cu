...
__device__ void NestedLoopFunction1(int i_spike, int i_target_rev_conn, bool all_connections, int _it)
{
  unsigned int target;
  if (all_connections) {
  ...
  if (syn_group>0) {
    ...
    unsigned short time_idx = (unsigned short)((NeuronGPUTimeIdx+_it) & 0xffff);
...
__device__ void InternRevSpikeBufferUpdate(unsigned int n_node, int _it)
{
  ...
  for (unsigned int i_node = threadIdx.x + offset; i_node<n_node && i_node < *nodes_per_block; i_node+=blockDim.x) { 
    long long target_spike_time_idx = LastRevSpikeTimeIdx[i_node];
    // Check if a spike reached the input synapses now
    if (target_spike_time_idx!=(NeuronGPUTimeIdx+_it)) {
      return;
    }
    int n_conn = TargetRevConnectionSize[i_node];
    if (n_conn>0) {
...
int RevSpikeFree()
{
  ...
  gpuErrchk(cudaFree(&d_InternRevSpikeNum));
  gpuErrchk(cudaFree(&d_InternRevSpikeTarget));
  gpuErrchk(cudaFree(&d_InternRevSpikeNConn));
